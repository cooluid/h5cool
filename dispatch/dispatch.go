...
var (
	...
	accountMsgs     = make(map[int]accountMsgHandler)
...
func RegAccountMsgHandle(cmdId int, handle func(*t.Account, *bytes.Reader)) {
	accountMsgs[cmdId] = handle
}

func RegActorMsgHandle(cmdId int, handle func(*t.Actor, *bytes.Reader)) {
	actorMsgs[cmdId] = handle
}
...
func PushClientMsg(account *t.Account, cmdId int, reader *bytes.Reader) {
	if cmdId <= maxAccountCmdId {
		...
		if _, ok := actorMsgs[cmdId]; ok {
			writerClientMsg(account, &message{mtType: mtClientActor, cbArgs: []interface{}{cmdId, reader, time.Now()}, actor: actor})
		}
	}
}
